...
try:
    from .clean_interactive_menu import (
...
class CleanHierarchicalSelector:
            ...
            "subfamilies": {
                "phi3": {
                    ...
                    "models": {
                        "phi3:mini": {
                            "name": "Phi-3 Mini",
                            "desc": "3.8B parameters • Lightweight • 4K context",
                            "icon": "⚡",
                        },
                        "phi3:medium": {
                            "name": "Phi-3 Medium",
                            "desc": "14B parameters • Strong reasoning • 4K context",
                            "icon": "🧠",
                        },
                        "phi3:medium-128k": {
                            "name": "Phi-3 Medium 128K",
                            "desc": "14B parameters • Strong reasoning • 128K context",
                            "icon": "🌐",
                        },
                    },
                },
            ...
            "subfamilies": {
                "gemma3": {
                    ...
                    "models": {
                        "gemma3:1b": {
                            "name": "Gemma 3 1B",
                            "desc": "1B parameters • Fast and lightweight • 32k context",
                            "icon": "⚡",
                        },
                        "gemma3:4b": {
                            "name": "Gemma 3 4B",
            ...
            "subfamilies": {
                "qwen3": {
                    ...
                    "models": {
                        "qwen3:1.7b": {
                            "name": "Qwen 3 1.7B",
                            "desc": "1.7B parameters • Efficient multilingual • 40K context",
                            "icon": "⚡",
                        },
                        "qwen3:8b": {
                            "name": "Qwen 3 8B",
                            "desc": "8B parameters • Strong performance • 40K context",
                            "icon": "🧠",
                        },
                    },
                }
    ...
    def display_model_families(self) -> Optional[str]:
        ...
        menu = CleanInteractiveMenu(
            "🤖 Select Model Family", "Choose the AI model family you want to explore:"
        )

        for key, data in self.MODEL_FAMILIES.items():
            menu.add_item(data["name"], data["description"], key, data["icon"])
    ...
    def display_models_in_subfamily(
        self, family_key: str, subfamily_key: str
    ) -> Optional[str]:
        """Display models with clean updates"""
        if (
            family_key not in self.MODEL_FAMILIES
            or subfamily_key not in self.MODEL_FAMILIES[family_key]["subfamilies"]
        ):
            return None
        ...
        for key, data in subfamily_data["models"].items():
            menu.add_item(data["name"], data["desc"], key, data["icon"])
    ...
    def show_final_selection(self, family_key: str, subfamily_key: str, model_key: str):
        ...
        print(
            f"{Colors.BRIGHT_CYAN}Family:{Colors.RESET}    {family_data['icon']} {family_data['name']}"
        )
        print(
            f"{Colors.BRIGHT_CYAN}Subfamily:{Colors.RESET} {subfamily_data['icon']} {subfamily_data['name']}"
        )
        print(
            f"{Colors.BRIGHT_CYAN}Model:{Colors.RESET}    {model_data['icon']} {model_data['name']}"
        )
        print(f"{Colors.BRIGHT_CYAN}Details:{Colors.RESET}   {model_data['desc']}")
        print(f"{Colors.BRIGHT_CYAN}{'=' * 60}{Colors.RESET}")
        print(
            f"{Colors.BRIGHT_GREEN}✨ Perfect choice for your AI needs!{Colors.RESET}"
        )
        print()
    ...
    def interactive_model_selection(self) -> Optional[str]:
        ...
        print(
            f"{Colors.BOLD}{Colors.BRIGHT_CYAN}🚀 Clean Ollama Model Selection{Colors.RESET}"
        )
        print(f"{Colors.BRIGHT_CYAN}{'=' * 60}{Colors.RESET}")
        print(
            f"{Colors.BRIGHT_YELLOW}💡 Clean display updates • No confusing logs below{Colors.RESET}"
        )
        print(f"{Colors.BRIGHT_CYAN}{'=' * 60}{Colors.RESET}")
        print()
