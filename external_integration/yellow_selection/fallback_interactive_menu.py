...
class FallbackInteractiveMenu:
    ...
    def add_item(
        self, display_name: str, description: str, value: Any, icon: str = "📋"
    ):
        self.items.append(
            {
    ...
    def _fallback_input(self) -> str:
        ...
        try:
            choice = input(
                f"{BRIGHT_YELLOW}Enter choice (1-{len(self.items)}) or 'q': {RESET}"
            ).strip()
            if choice.lower() == "q":
                return "q"
    ...
    def display_menu(self):
        """Display the complete menu with current selection highlighted and search filter"""
        # Clear screen
        ...
        for display_idx, idx in enumerate(filtered):
            ...
            if display_idx == self.current_index:
                ...
                print(
                    f"{BG_YELLOW}{BLACK}  ▶ [{idx+1}] {item['icon']} {item['display_name']}{RESET}"
                )
        ...
        if self.filter_text:
            print(
                f"{BRIGHT_GREEN}  🔍 Filter: '{self.filter_text}' ({len(filtered)}/{len(self.items)} items){RESET}"
            )
        else:
            print(f"{BRIGHT_YELLOW}  Type to search{RESET}")
        print()

        # Display footer
        print(f"{BRIGHT_CYAN}{'─' * 50}{RESET}")
        print(
            f"{BRIGHT_YELLOW}💡 Use ↑↓ arrows • 1-9 numbers • Type to search • Enter to select • BS:Clear • 'q' to quit{RESET}"
        )
        print()
        print()

    def show(self) -> Optional[Any]:
        """Show interactive menu with fallback display updates and search filter"""
        try:
            # Initial display
            self.display_menu()
    ...
    def fallback_selection(self) -> Optional[Any]:
        ...
        if filter_text:
            print(
                f"{BRIGHT_GREEN}  🔍 Filter: '{filter_text}' ({len(filtered)}/{len(self.items)} items){RESET}"
            )
            print()
        ...
        while True:
            try:
                choice = input(
                    f"{YELLOW}Select (1-{len(filtered)}) or 'q': {RESET}"
                ).strip()
                if choice.lower() == "q":
                    print("\033[2J\033[H", end="", flush=True)
                    return None
