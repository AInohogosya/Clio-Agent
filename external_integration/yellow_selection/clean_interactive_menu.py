...
class CleanInteractiveMenu:
    ...
    def display_header(self):
        ...
        if not self.displayed_once:
            ...
            print()
            print(
                f"{Colors.BRIGHT_YELLOW}💡 Use ↑↓ arrows • 1-9 numbers • Type to search • Enter to select • 'q' to quit{Colors.RESET}"
            )
            print()
            print()
    ...
    def update_display(self):
        ...
        if self.filter_text:
            print(
                f"{Colors.BRIGHT_GREEN}  🔍 Filter: '{self.filter_text}_'  ({len(filtered)}/{len(self.items)} items){Colors.RESET}",
                end="",
                flush=True,
            )
        else:
        ...
        for display_idx, idx in enumerate(filtered):
            ...
            if display_idx == self.current_index:
                print(
                    f"{Colors.BG_YELLOW}{Colors.BLACK}  ▶ [{idx+1}] {item['icon']} {item['display_name']}{Colors.RESET}",
                    end="",
                    flush=True,
                )
            else:
                print(
                    f"  [{idx+1}] {item['icon']} {item['display_name']}",
                    end="",
                    flush=True,
                )
            ...
            if display_idx == self.current_index:
                print(
                    f"{Colors.BG_YELLOW}{Colors.BLACK}       {item['description']}{Colors.RESET}",
                    end="",
                    flush=True,
                )
            else:
    ...
    def fallback_selection(self) -> Optional[Any]:
        ...
        if filter_text:
            print(
                f"{Colors.BRIGHT_GREEN}  🔍 Filter: '{filter_text}' ({len(filtered)}/{len(self.items)} items){Colors.RESET}"
            )
            print()
        ...
        while True:
            try:
                prompt = (
                    f"{Colors.YELLOW}Select (1-{len(filtered)}) or 'q': {Colors.RESET}"
                )
