    ...
    def mask_dict(self, data: Dict) -> Dict:
        ...
        for key, value in data.items():
            ...
            if any(
                ...
                for sensitive in ["key", "password", "token", "secret", "credential"]
            ):
                ...
                result[key] = [
                    self.mask(str(item)) if isinstance(item, str) else item
                    for item in value
                ]
            else:
                result[key] = value
...
class SandboxManager:
    ...
    def _detect_sandbox_tool(self) -> Optional[str]:
        ...
        self.logger.warning(
            "No sandboxing tool detected. Running commands in unrestricted environment."
        )
        return None
    ...
    def wrap_command(self, command: str, workspace: Optional[Path] = None) -> str:
        ...
        if not self.available_sandbox:
            self.logger.warning(
                "No sandbox tool available, running command without restrictions"
            )
            return command
...
class SecurityManager:
    ...
    def __init__(
        ...
        self.sandbox = (
            SandboxManager()
            if (enable_sandbox and self.config.enable_sandbox)
            else None
        )
    ...
    def validate_and_prepare(self, commands: List[str]) -> tuple[List[str], List[str]]:
        ...
        for cmd in commands:
            ...
            if result.requires_confirmation:
                self.logger.warning(
                    f"Command requires confirmation: {result.masked_output}"
                )
...
def get_security_manager(
    ...
    if _global_security_manager is None:
        ...
        _global_security_manager = SecurityManager(
            config, enable_sandbox, auto_mask_logs
        )
