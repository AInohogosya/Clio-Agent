...
class ProviderFallbackManager:
    ...
    def get_next_available_provider(
        self, preferred_provider: str, excluded: Optional[List[str]] = None
        ...
        if preferred_provider not in excluded:
            ...
            if (
                health
                and health.status == ProviderStatus.HEALTHY
    ...
    def execute_with_fallback(
        ...
        **kwargs,
        ...
        max_total_attempts = (
            len(self.config.fallback_order) * self.config.max_retries_per_provider
        )
        total_attempts = 0
        ...
        while current_provider and total_attempts < max_total_attempts:
            ...
            if health and health.is_circuit_open:
                ...
                current_provider, _ = self.get_next_available_provider(
                    preferred_provider, excluded=attempted_providers
                )
                continue

            try:
                self.logger.info(
                    f"Attempting execution with {current_provider}/{current_model}"
                )
                start_time = time.time()

                # Execute the function
                result = execute_func(
                    provider=current_provider, model=current_model, *args, **kwargs
                )
                ...
                self.logger.info(
                    f"Execution successful with {current_provider}", latency=latency
                )
                ...
                if (
                    ...
                    and self._get_health(current_provider).consecutive_failures
