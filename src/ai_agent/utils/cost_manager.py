...
class CostManager:
    ...
    PRICING: Dict[str, ModelPricing] = {
        ...
        "gpt-5.4-mini": ModelPricing("openai", "gpt-5.4-mini", 0.60, 2.40, 128000),
        # Google
        "gemini-3.1-pro": ModelPricing(
            "google", "gemini-3.1-pro", 2.50, 15.00, 2000000
        ),
        "gemini-3-flash": ModelPricing(
            "google", "gemini-3-flash", 0.125, 0.375, 1000000
        ),
        # Anthropic
        "claude-opus-4.6": ModelPricing(
            "anthropic", "claude-opus-4.6", 15.00, 75.00, 200000
        ),
        "claude-sonnet-4.6": ModelPricing(
            "anthropic", "claude-sonnet-4.6", 3.00, 15.00, 200000
        ),
        # Groq (typically cheaper/faster)
        "llama-3.3-70b-versatile": ModelPricing(
            "groq", "llama-3.3-70b-versatile", 0.59, 0.79, 128000
        ),
    ...
    def __init__(
        self, config: Optional[BudgetConfig] = None, persist_path: Optional[str] = None
    ):
        ...
        self.persist_path = (
            Path(persist_path)
            if persist_path
            else Path.home() / ".clio_agent" / "costs.json"
        )
    ...
    def record_usage(
        ...
        with self.lock:
            ...
            self.stats.daily_costs[today] = (
                self.stats.daily_costs.get(today, 0.0) + cost
            )
            self.stats.monthly_costs[month] = (
                self.stats.monthly_costs.get(month, 0.0) + cost
            )

            # Provider and model tracking
            provider_key = f"{provider}/{model}"
            self.stats.provider_costs[provider] = (
                self.stats.provider_costs.get(provider, 0.0) + cost
            )
            self.stats.model_costs[provider_key] = (
                self.stats.model_costs.get(provider_key, 0.0) + cost
            )
    ...
    def check_budget(self, estimated_cost: float) -> tuple[bool, Optional[str]]:
        ...
        if (
            self.config.per_request_budget
            and estimated_cost > self.config.per_request_budget
        ):
            return (
                False,
                f"Estimated cost ${estimated_cost:.4f} exceeds per-request budget ${self.config.per_request_budget:.4f}",
            )
        ...
        if self.config.daily_budget:
            ...
            if daily_spent + estimated_cost > self.config.daily_budget:
                return (
                    False,
                    f"Daily budget ${self.config.daily_budget:.2f} would be exceeded",
                )
        ...
        if self.config.monthly_budget:
            ...
            if monthly_spent + estimated_cost > self.config.monthly_budget:
                return (
                    False,
                    f"Monthly budget ${self.config.monthly_budget:.2f} would be exceeded",
                )

        return True, None

    def get_cheaper_alternative(
        self, provider: str, model: str, max_quality_degradation: str = "minimal"
        ...
        current_cost = (
            current_pricing.input_price_per_1m + current_pricing.output_price_per_1m
        )
        ...
        for key, pricing in self.PRICING.items():
            ...
            quality_ok = self._check_quality_compatibility(
                model, pricing.model, max_quality_degradation
            )
    ...
    def get_budget_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "today": {
                ...
                "percentage": (
                    (daily_spent / self.config.daily_budget * 100)
                    if self.config.daily_budget
                    else None
                ),
            },
            "this_month": {
                ...
                "percentage": (
                    (monthly_spent / self.config.monthly_budget * 100)
                    if self.config.monthly_budget
                    else None
                ),
            },
    ...
    def get_usage_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_providers": dict(
                sorted(
                    self.stats.provider_costs.items(), key=lambda x: x[1], reverse=True
                )[:5]
            ),
            "top_models": dict(
                sorted(
                    self.stats.model_costs.items(), key=lambda x: x[1], reverse=True
                )[:5]
            ),
        }
    ...
    def _get_pricing(self, provider: str, model: str) -> ModelPricing:
        ...
        # Try fuzzy match
        for pkey, pricing in self.PRICING.items():
            if pricing.provider == provider and model.startswith(
                pricing.model.split(":")[0]
            ):
                return pricing
    ...
    def _check_quality_compatibility(
        self, original: str, alternative: str, max_degradation: str
    ) -> bool:
    ...
    def _get_alert_level(
        self, daily_spent: float, monthly_spent: float
    ) -> BudgetAlertLevel:
        ...
        if self.config.monthly_budget:
            max_percentage = max(
                max_percentage, monthly_spent / self.config.monthly_budget
            )
    ...
    def _load_from_disk(self):
        ...
        try:
            ...
            self.stats.total_completion_tokens = stats_data.get(
            ...
            self.logger.info(
                f"Loaded cost data: ${self.stats.total_cost:.4f} total spent"
            )
...
def get_cost_manager(
    daily_budget: Optional[float] = None, monthly_budget: Optional[float] = None
) -> CostManager:
    """Get global cost manager instance"""
    global _global_cost_manager

    if _global_cost_manager is None:
        config = BudgetConfig(daily_budget=daily_budget, monthly_budget=monthly_budget)
