...
class DependencyChecker:
    ...
    def __init__(self, project_root: Path):
        ...
        self.platform_dependencies = {
            "darwin": {"objc": "pyobjc-framework-Cocoa>=9.0"},
            "win32": {"win32api": "pywin32>=306"},
            "linux": {"Xlib": "python-xlib>=0.33"},
        }
    ...
    def check_python_version(self) -> Tuple[bool, str]:
        ...
        if current_version >= required_version:
            ...
            return (
                False,
                f"Python {current_version[0]}.{current_version[1]} (required >=3.8) ✗",
            )
    ...
    def install_package(
        self, package: str, retries: int = 3, use_venv: bool = True
    ) -> Tuple[bool, str]:
        ...
        if not use_venv:
            return (
                False,
                "System Python installation is not allowed. Virtual environment is required.",
            )
        ...
        if not pip_cmd:
            return (
                False,
                "Virtual environment pip not available. Please recreate the virtual environment.",
            )
            ...
            for attempt in range(retries):
                try:
                    if attempt > 0:
                        ...
                        print(
                            f"🔄 Retry {attempt + 1}/{retries} for {package} (strategy {strategy_idx + 1})..."
                        )
                        ...
                        if strategy_idx == 0:
                            ...
                            print(
                                f"🔄 Trying alternative installation for {package} (strategy {strategy_idx + 1})..."
                            )
                    ...
                    if result.returncode == 0:
                        ...
                        if "Permission denied" in error_msg:
                            return (
                                False,
                                f"Permission denied installing {package}. Virtual environment may have permission issues.",
                            )
                        elif (
                            "Could not find a version" in error_msg
                            or "404" in error_msg
                        ):
                            return (
                                False,
                                f"Package {package} not found or version incompatible.",
                            )
                        elif (
                            ...
                            if strategy_idx < len(install_strategies) - 1:
                                ...
                                return (
                                    False,
                                    f"Network error installing {package} after all strategies. Check internet connection.",
                                )
                            else:
                                continue
                            ...
                            if strategy_idx < len(install_strategies) - 1:
                                ...
                                return (
                                    False,
                                    f"Installation of {package} timed out after all strategies.",
                                )
                            else:
                                continue
                        elif (
                            attempt == retries - 1
                            and strategy_idx == len(install_strategies) - 1
                        ):
                            return (
                                False,
                                f"Failed to install {package} after all strategies and retries: {error_msg[:200]}",
                            )
                        else:
                            continue
    ...
    def install_requirements_file(
        self, retries: int = 2, use_venv: bool = True
    ) -> Tuple[bool, str]:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if attempt == retries - 1:
                        return (
                            False,
                            f"Failed to install requirements.txt after {retries} attempts: {error_msg}",
                        )
                    else:
                ...
                if attempt == retries - 1:
                    return (
                        False,
                        f"Installation of requirements.txt timed out after {retries} attempts",
                    )
                time.sleep(5)
                continue
            except Exception as e:
    ...
    def install_project(
        ...
        for attempt in range(retries):
            try:
                if attempt > 0:
                    print(
                        f"🔄 Retry {attempt + 1}/{retries} for project installation..."
                    )
                else:
                ...
                if result.returncode == 0:
                    ...
                    if attempt == retries - 1:
                        return (
                            False,
                            f"Failed to install project after {retries} attempts: {error_msg}",
                        )
                    else:
                        time.sleep(3)
                        continue
                ...
                if attempt == retries - 1:
                    return (
                        False,
                        f"Project installation timed out after {retries} attempts",
                    )
                time.sleep(5)
                continue
            except Exception as e:
            ...
            try:
                with open("/etc/os-release", "r") as f:
                    ...
                    if "debian" in os_release.lower() or "ubuntu" in os_release.lower():
                        ...
                        "rhel" in os_release.lower()
                        or "centos" in os_release.lower()
                        or "fedora" in os_release.lower()
                    ):
                        distro = "redhat"
                    else:
                        distro = "unknown"
    ...
    def auto_install_missing(self, missing_deps: List[str]) -> bool:
        ...
        if conditional_missing:
            ...
            self._install_dependencies(
                conditional_missing, self.conditional_dependencies, optional=True
            )

        return True

    def _install_dependencies(
        self, missing_deps: List[str], dep_dict: Dict[str, str], optional: bool = False
    ) -> bool:
        ...
        if not venv_ok:
            print(
                "🔧 ERROR: Not in virtual environment. Creating one for complete dependency isolation..."
            )
            ...
            if venv_success:
                ...
                print(f"❌ {venv_message}")
                print(
                    "❌ ERROR: Virtual environment is required. Cannot proceed with system Python."
                )
                return False
        else:
            use_venv = True
        ...
        for dep in missing_deps:
            ...
            if success:
                ...
                if not optional:
                    ...
                    print(
                        f"⚠️  Optional dependency {dep} failed to install, continuing..."
                    )
    ...
    def run_full_check(self, auto_install: bool = True) -> bool:
        ...
        print("🚀 Starting dependency check for Clio-Agent-1 AI Agent\n")
        ...
        missing_conditional = [
            mod for mod in missing_core if mod in self.conditional_dependencies
        ]
        missing_core_only = [
            mod for mod in missing_core if mod not in self.conditional_dependencies
        ]
        ...
        print(
            f"   Conditional dependencies missing: {len(missing_conditional)} (optional)"
        )
        ...
        if auto_install and all_missing:
            ...
            if success:
                ...
                # Final verification
                final_missing = [
                    mod for mod in all_missing if not self.check_import(mod)
                ]
