#!/usr/bin/env python3
"""
Environment Detection and Adaptive Execution System for Clio-Agent-1-2.2
...
class EnvironmentDetector:
    ...
    def _detect_venv_module(self) -> bool:
        ...
        try:
            ...
            self.suggestions.append(
                "Install python3-venv package (e.g., 'sudo apt install python3-venv')"
            )
            return False
    ...
    def _detect_ollama_has_whoami(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["ollama", "whoami"], capture_output=True, text=True, timeout=10
            )
    ...
    def _detect_needs_ollama_update(self) -> bool:
        ...
        try:
            ...
            if major == 0 and minor < 17:
                self.suggestions.append(
                    f"Update Ollama to 0.17.0+ for cloud model support (current: {version})"
                )
                return True
            return False
        except Exception:
...
class AdaptiveExecutor:
    ...
    def create_execution_plan(self) -> List[Dict[str, Any]]:
        ...
        if (
            self.env.ollama_available
            and self.env.ollama_has_signin
            and not self.env.can_use_cloud_models
        ):
            plan.append(
                {
                    "step": 2,
    ...
    def execute_plan(self, interactive: bool = True) -> bool:
        ...
        for step in self.execution_plan:
            ...
            if interactive:
                if step["can_skip"]:
                    response = (
                        input(f"  Execute this step? [Y/n/skip]: ").lower().strip()
                    )
                    if response in ["n", "no"]:
                        print("  ⏭ Skipped")
                        continue
                ...
                if "curl" in cmd and "| sh" in cmd:
                    ...
                    if url:
                        ...
                        result = type(
                            "obj",
                            (object,),
                            {"returncode": -1, "stderr": "Could not extract URL"},
                        )()
                    ...
                    is_allowed = any(
                        cmd.startswith(ac) or f" {ac} " in cmd
                    ...
                    if is_allowed:
                        ...
                        print(
                            f"  ⚠ Skipped: Command contains potentially unsafe patterns"
                        )
                        step["failed"] = True
                        continue
                else:
...
def detect_and_plan() -> Tuple[EnvironmentInfo, AdaptiveExecutor]:
    ...
    print(
        f"  Ollama: {'✓' if env_info.ollama_available else '✗'} {env_info.ollama_version or 'N/A'}"
    )
