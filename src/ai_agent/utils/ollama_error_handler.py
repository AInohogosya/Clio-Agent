"""
Enhanced Ollama Error Handler for Clio-Agent-1-2.2
...
class OllamaErrorHandler:
        ...
        for pattern, handler in self.error_patterns.items():
            if re.search(pattern, error_lower, re.IGNORECASE):
                try:
                    ...
                    if self.logger:
                        self.logger.warning(
                            f"Error handler failed for pattern {pattern}: {e}"
                        )
                    continue
    ...
    def _handle_model_not_found(
        ...
        if model_name == "unknown":
            match = re.search(
                r'model ["\']?([^"\':\s]+)["\']?', error_message, re.IGNORECASE
            )
            if match:
