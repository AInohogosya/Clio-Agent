"""
Automatic SDK Installer for Clio-Agent-1
...
class SDKInstaller:
    ...
    def install_sdk(self, provider: str, interactive: bool = True) -> bool:
        ...
        for strategy_idx, install_cmd in enumerate(install_strategies):
            try:
                if strategy_idx > 0:
                    print(
                        f"🔄 Trying alternative installation strategy {strategy_idx + 1}..."
                    )
                ...
                if result.returncode == 0:
                    ...
                    if "Permission denied" in error_msg:
                        ...
                        print(
                            "   Try running in a virtual environment or check permissions"
                        )
                        break  # Don't retry permission errors
                    elif "Could not find a version" in error_msg or "404" in error_msg:
                        print(
                            f"❌ Package {sdk_info['package']} not found or incompatible"
                        )
    ...
    def install_missing_sdks(
        self, providers: List[str], interactive: bool = True
    ) -> Dict[str, bool]:
