...
from .model_definitions import (
    ...
    get_model_families,
    get_subfamilies,
    get_models_in_subfamily,
    get_model_hierarchy_path,
...
class OllamaManager:
    ...
    def validate_model(self, model_name: str) -> Dict[str, Any]:
        ...
        is_predefined = model_name in PREDEFINED_MODELS
    ...
    def install_model(self, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if return_code == 0:
                ...
                context = {"model_name": model_name, "operation": "install_model"}
                try:
                    handle_ollama_error(
                        stderr or "Installation failed", context, display_to_user=True
                    )
                except Exception as handler_error:
                    self.logger.warning(
                        f"Enhanced error handler failed: {handler_error}"
                    )
            ...
            try:
                handle_ollama_error(
                    f"Installation of {model_name} timed out",
                    context,
    ...
    def display_model_families(self) -> Optional[str]:
        ...
        try:
            ...
            for key, data in families.items():
                icon = data.get("icon", "🤖")
                menu.add_item(data["name"], data["description"], key, icon)
    ...
    def _fallback_model_families(self) -> Optional[str]:
        ...
        while True:
            try:
                choice = input(
                    f"\nSelect model family (1-{len(family_keys)}) or 'q' to quit: "
                ).strip()
                if choice.lower() == "q":
                    return None
    ...
    def _fallback_subfamilies(self, family_key: str) -> Optional[str]:
        ...
        while True:
            try:
                choice = input(
                    f"\nSelect subfamily (1-{len(subfamily_keys)}) or 'q' to quit: "
                ).strip()
                if choice.lower() == "q":
                    return None
    ...
    def display_models_in_subfamily(
        ...
        try:
            ...
            for key, model_data in models.items():
                ...
                menu.add_item(model_data.get("name", key), description, key, icon)
    ...
    def _fallback_models_in_subfamily(
        self, family_key: str, subfamily_key: str
    ) -> Optional[str]:
        """Fallback method using original number-based selection"""
        models = self.get_models_in_subfamily(family_key, subfamily_key)
        ...
        while True:
            try:
                choice = input(
                    f"\nSelect model (1-{len(model_keys)}) or 'q' to quit: "
                ).strip()
                if choice.lower() == "q":
                    return None
    ...
    def demo_hierarchical_selection(self):
        ...
        for family_key, family_data in families.items():
            ...
            for subfamily_key, subfamily_data in subfamilies.items():
                ...
                print(
                    f"    📂 {subfamily_icon} {subfamily_data['name']} ({subfamily_key}) - {model_count} models"
                )
    ...
    def suggest_model_installation(self, model_name: str) -> bool:
        ...
        if not self.is_model_installed(model_name):
            ...
            if validation["valid"]:
                ...
                response = (
                    input("Would you like to install it now? (y/n): ").lower().strip()
                )
