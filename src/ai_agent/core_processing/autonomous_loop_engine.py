...
class AutonomousLoopEngine:
    ...
    def _run_thinking(self, ctx: AutonomousContext) -> str:
        ...
        if ctx.discord_mode:
            discord_section = (
                ...
                "3. COMPLETION: When done, send discord() with a summary of what was done.\n"
    ...
    def _build_system_instruction(self, ctx: AutonomousContext) -> str:
        ...
        if ctx.telegram_mode:
            base += (
                "\n## 5. 📱 TELEGRAM MODE (ACTIVE — FOLLOW STRICTLY)\n"
                "You are communicating via Telegram. The user CANNOT see your terminal,\n"
                ...
                "   Example: telegram(⏳ Still working on [task]... iteration #[N].)\n\n"
                "3. COMPLETION: When done, ALWAYS send a telegram() summary.\n"
                "   Example: telegram(✅ Done! Created file X, fixed bug Y.)\n\n"
                "4. ERRORS: On failure, immediately notify via telegram().\n"
                "   Example: telegram(❌ Error: [description]. Attempting recovery...)\n\n"
                "5. thinking() is a BLACK HOLE — user CANNOT see it. Never put\n"
                "   user-facing messages in thinking(). Use telegram() for ALL user comms.\n\n"
                "6. OVER-COMMUNICATE: When in doubt, send telegram(). Silence > verbose plans.\n\n"
                "7. FORMATTING: Use \\n for newlines, emoji for clarity (✅ ❌ ⏳ 🔄 📁 🔍).\n"
                "   Keep messages under 4000 chars; split long messages into multiple calls.\n\n"
                "8. DO NOT spam trivial updates. DO NOT send empty/meaningless telegram() calls.\n"
                "   DO NOT use telegram() for internal notes (use thinking() for that).\n"
            )
        if ctx.discord_mode:
            base += (
                "\n## 5. 💬 DISCORD MODE (ACTIVE — FOLLOW STRICTLY)\n"
                "You are communicating via Discord. The user CANNOT see your terminal,\n"
                "your execution log, or your thinking() output. discord() is the ONLY\n"
                "way to send messages to the user. If you don't use discord(), the user\n"
                "receives NOTHING.\n\n"
                "### Discord Command Rules:\n"
                "1. IMMEDIATE ACKNOWLEDGMENT: When a user message appears in your log,\n"
                "   your VERY FIRST action MUST be discord() with an acknowledgement.\n"
                "   Example: discord(👋 Got it! Working on it now...)\n"
                "   Then do the actual work. Never leave the user waiting.\n\n"
                "2. PROGRESS UPDATES: Send discord() updates every 5-10 iterations.\n"
                "   NEVER go more than 10 iterations without a discord() message.\n"
                "   Example: discord(⏳ Still working on [task]... iteration #[N].)\n\n"
                "3. COMPLETION: When done, ALWAYS send a discord() summary.\n"
                "   Example: discord(✅ Done! Created file X, fixed bug Y.)\n\n"
                "4. ERRORS: On failure, immediately notify via discord().\n"
                "   Example: discord(❌ Error: [description]. Attempting recovery...)\n\n"
                "5. thinking() is a BLACK HOLE — user CANNOT see it. Never put\n"
                "   user-facing messages in thinking(). Use discord() for ALL user comms.\n\n"
                "6. OVER-COMMUNICATE: When in doubt, send discord(). Silence > verbose plans.\n\n"
                "7. FORMATTING: Use \\n for newlines, emoji for clarity (✅ ❌ ⏳ 🔄 📁 🔍).\n"
                "   Keep messages under 2000 chars; split long messages into multiple calls.\n\n"
                "8. DO NOT spam trivial updates. DO NOT send empty/meaningless discord() calls.\n"
                "   DO NOT use discord() for internal notes (use thinking() for that).\n"
            )
