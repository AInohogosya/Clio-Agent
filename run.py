...
def _inspect_venv_deps(venv_python):
    ...
    check_script = (
        ...
        "    try:\n"
        "        _spec = importlib.util.find_spec(mod)\n"
...
def _auto_fix_venv_failure(err="", sudo_password=None):
    ...
    if sys.platform == "darwin":
        ...
        if not fixed:
            ...
            if _run_fix_command(_virtualenv_cmd, timeout=120):
                fixed = True
...
def _create_venv_and_install(sudo_password=None):
    ...
    if not venv_created:
        ...
        try:
            # BUG FIX: --user installs fail inside an active virtualenv.
            _virtualenv_install_cmd = [sys.executable, "-m", "pip", "install", "virtualenv"]
            if not _is_in_venv():
                _virtualenv_install_cmd.insert(4, "--user")
            r = subprocess.run(_virtualenv_install_cmd,
    ...
    if not venv_created:
        ...
        if _auto_fix_venv_failure(last_err, sudo_password=sudo_password):
            ...
            if not venv_created:
                try:
                    # BUG FIX: --user installs fail inside an active virtualenv.
                    _virtualenv_install_cmd2 = [sys.executable, "-m", "pip", "install", "virtualenv"]
                    if not _is_in_venv():
                        _virtualenv_install_cmd2.insert(4, "--user")
                    subprocess.run(
                        _virtualenv_install_cmd2,
                        capture_output=True, text=True, timeout=60,
                    )
